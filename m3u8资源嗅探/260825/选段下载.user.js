...
(function () {
    ...
    // m3u8分片下载（边下边存 + 释放内存）
    // ==========================================
    async function downloadM3u8BySegments(segments, keyCache, onProgress, writer, segmentIndices = null, onSegmentDone = null) {
        ...
        // 时间戳修复状态（跨分片共享，必须按顺序修复）
        let baseTime = null;

        function updateProgress() {
            const avgProgress = progressMap.reduce((a, b) => a + b, 0) / total;
            const doneCount = progressMap.filter(p => p >= 1).length;
            onProgress(avgProgress * 100, `${(avgProgress * 100).toFixed(1)}% | ${doneCount}/${total} | ${formatBytes(downloadedBytes)}`);
        }

        // 修复时间戳并写入writer
        function fixAndWrite(buf) {
            ...
            for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
                try {
                    ...
                    const data = await gmRequest(seg.url, true, (loaded, totalSize) => {
                        progressMap[idx] = totalSize ? (loaded / totalSize) * 0.9 : 0.5;
                        updateProgress();
                    });
        ...
        for (let i = 0; i < total; i += CONCURRENCY) {
            ...
            for (let j = 0; j < results.length; j++) {
                ...
                const segIdx = segmentIndices ? segmentIndices[i + j] : (i + j);
                progressMap[i + j] = 1;
                completed++;
                updateProgress();
    ...
    window.TaskRunner = async function (url, type, btn, opt = {}) {
        ...
        try {
            ...
            if (opt.ranges && opt.ranges.length > 0) {
                ...
                await downloadM3u8BySegments(parseRet.segments, keyCache, (p, txt) => { btn.textContent = txt; }, writer, null, opt.onSegmentDone);
    ...
    async function openPreviewSelectSegment(m3u8Url, existingSegments = []) {
        ...
        startSlider.oninput = () => {
            ...
            startSec = Math.max(0, Math.min(val, sliderMax));
            startLabel.textContent = `起始：${formatTimeHMS(startSec)}`;
        ...
        endSlider.oninput = () => {
            ...
            endSec = Math.max(0, Math.min(val, sliderMax));
            endLabel.textContent = `结束：${formatTimeHMS(endSec)}`;
        ...
        quickBtns.forEach(btn => {
            ...
            button.onclick = () => {
                if (activeSlider === 'start') {
                    startSec = Math.max(0, Math.min(startSec + btn.delta, sliderMax));
                    startSlider.value = String(startSec);
                    startLabel.textContent = `起始：${formatTimeHMS(startSec)}`;
                    video.currentTime = startSec;
                } else {
                    endSec = Math.max(0, Math.min(endSec + btn.delta, sliderMax));
                    endSlider.value = String(endSec);
                    endLabel.textContent = `结束：${formatTimeHMS(endSec)}`;
                    video.currentTime = endSec;
                }
                ...
                if (isOpen) {
                    ...
                    if (segCount > 0) {
                        ...
                        item._cutSegments.forEach((seg, i) => {
                            const isDownloaded = seg.startIdx != null && seg.endIdx != null &&
                                item._downloadedSegments &&
